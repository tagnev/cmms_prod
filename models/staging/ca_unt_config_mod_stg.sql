{{
    config(
        materialized="incremental", unique_key=["mfgr_mdl_num", "mfgr_id", "sr_num"]
    )
}}

with
    ca_unt_config_mod_raw_stg as (
        select
    ...
    calculations as (
        ...
        from ca_unt_config_mod_raw_stg raw_stg
        left outer join
    ...
    final as (
        select
            (
                case
                    when unt_config_mod_id is null
                    then (select ca_unt_config_mod_stg_seq.nextval)
                    else unt_config_mod_id
                end
            ) as unt_config_mod_id,
            mfgr_mdl_num,
            agt_mdl_num,
            unt_descr as descr,
            sr_num,
